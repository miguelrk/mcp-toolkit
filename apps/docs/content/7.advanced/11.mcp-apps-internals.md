...
The bundle is an isolated Vue app, not a Nuxt page. To add Vue-only integrations such as
Nuxt UI, customize that build without sharing the host Nuxt graph:
...
Use the Vue-only Nuxt UI install (`@nuxt/ui/vite` and `@nuxt/ui/vue-plugin`), not the Nuxt
module. The iframe has no Nuxt runtime, auto-imports, cookies, or shared module graph.
`css` imports are inlined into the HTML bundle, and `~`/`@` resolve from the Nuxt source
directory. Additional `plugins` run between the toolkit's required Vue and single-file
plugins, which cannot be removed. The mount node includes `isolate` by default.
